...
def buildCityGraph(graphType):
    g = graphType()
    for name in ('Rimnicu Vilcea','Arad', 'Sibiu', 'Zerind', 'Oradea', 'Timisoara', 'Lugoj', 'Mehadia', 'Dobreta', 'Craiova','Pitesti', 'Fagaras', 'Bucarest'):
        g.addNode(Node(name))
    ...
    g.addEdge(Edge(g.getNode('Timisoara'), g.getNode('Arad')))
    ...
    g.addEdge(Edge(g.getNode('Sibiu'), g.getNode('Rimnicu Vilcea')))
...
def testSP(source, destination):
    ...
    if sp != None:
        print('\nCamino más corto desde ', source, 'a',
              destination, 'es', printPath(sp),'\n')
    else:
        print('\nNo hay camino desde', source, 'a', destination)
        
testSP('Lugoj', 'Bucarest')
